...
-- 5. Write a query against the orders table that displays the total number of distinct customers who have placed orders, by customer ID, for each country
-- orders have been shipped. Again, use an alias to label the count as CustomerCount. Order the list by the CustomerCount, largest to smallest.
...
SELECT Title,COUNT(DISTINCT EmployeeID) AS Employees
FROM Employees 
GROUP BY Title;
